...
var parseTorrents = function(body, callback) {
  ...
  table.find('tr').each(function() {
    ...
    if (tr.is('.group_redline, .group')) {
      ...
      var td3a = tds.eq(3).children('a');

      // artist
      t.artist = parseArtist(td3a.first());

      // relesae info
      var parsedHref = url.parse(ent.decode(td3a.eq(1).attr('href')), true);
      var n = t.artist === null ? 0 : 1;
    ...
    // check if this is a subgroup
      ...
      // torrent info
      var tAnchor = tds.first().find('a').eq(2);
      var parsedHref = url.parse(ent.decode(tAnchor.attr('href')), true);
    ...
    // single torrent
      ...
      var td3a = tds.eq(3).find('a');

      // artist
      t.artist = parseArtist(td3a.eq(2));

      var n = t.artist === null ? 2 : 3;
...
var parseArtist = function(a) {
  ...
  var rs = /^(.+)?( \(View Artist\))|(View Artist)$/.exec(a.attr('title'));
...
var parseRelease = function(id, a, tds, single) {
  ...
  var title = ent.decode(a.text().trim())
    , rs = /^(.+)?( \(View Torrent\))|(View Torrent)$/
             .exec(a.attr('title'))
    , orgtitle = rs[1] || null
    , td3split = tds.eq(3).text().trim().split('\t\t\t')
    , regex, data, str, date, comments, tags
