/*jshint jquery:false, latedef:false */
var url     = require('url');
...
var parseRelease = function(id, a, tds, single) {
  ...
  var td3split = tds.eq(3).text().trim().split('\t\t\t');
  var rs, regex, data, str, date, comments, tags;
...
var parseDate = function(title, date) {
  ...
  var rs = regex.exec(title);
  return date || (rs !== null ? rs[1] : null);
};


var formats = [
  'MP3', 'FLAC', 'TAK', 'TTA', 'ALAC', 'Ogg Orbis', 'APE', 'AAC', 'WMA',
  'AC3', 'WavPack', 'DTS', 'IMG', 'ISO', 'VOB', 'MPEG', 'MPEG2', 'AVI',
  'MKV', 'WMV', 'MP4', 'h264', 'Ogg', 'WAV'
].join('|');

var bitrates = [
  '192', 'V2 \\(VBR\\)', 'V1 \\(VBR\\)', '256', 'V0 \\(VBR\\)', '320',
  'Lossless', 'Variable', 'Other'
].join('|');

var medias = [
  'CD', 'DVD', 'Blu-Ray', 'VHS', 'VCD', 'TV', 'HDTV',
  'Radio', 'Vinyl', 'WEB'
].join('|');

var torrentRegex = new RegExp(
  '(' + formats + ')( \/ (' + bitrates + '))?( \/ (' + medias + '))?' +
  '( \/ (.+))?( \/ Freeleech!)?');
